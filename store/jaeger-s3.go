...
var (
        _ shared.StoragePlugin = (*Storag)(nil)
        _ io.Closer             = (*Storag)(nil)
)
...
func (c *Config) Validate() error {
	...
	if err := c.Querier.Validate(); err != nil {
		return errors.Wrap(err, "inalid querier config")
	}
...
func New(cfg Config, logger hclog.Logger) (*Loki, *Storag, func() error, error) {
	...
	if err := loki.setupModuleManager(); err != nil {
		return loki, nil, nil, err
	}
	storage.RegisterCustomIndexClients(&loki.Cfg.StorageConfig, loki.clientMetrics, prometheus.DefaultRegisterer)

        limits, err := validation.NewOverrides(loki.Cfg.LimitsConfig, nil)
        if err != nil {
                return loki, nil, nil, err
        }

        chunkStore, err := chunk_storage.NewStore(loki.Cfg.StorageConfig.Config, loki.Cfg.ChunkStoreConfig.StoreConfig, loki.Cfg.SchemaConfig.SchemaConfig, limits, loki.clientMetrics, nil, nil, util_log.Logger)
        if err != nil {
                return loki, nil, nil, err
        }

        querier, err := storage.NewStore(loki.Cfg.StorageConfig, loki.Cfg.SchemaConfig, chunkStore, nil)
        if err != nil {
                return loki, nil, nil, err
        }

        /* ingesterr, err := ingester.New(loki.Cfg.Ingester, loki.Cfg.IngesterClient, querier, limits, runtime.DefaultTenantConfigs(), nil)  
        if err != nil {
                return loki, nil, nil, err
        } */

        /* distributorr, err := distributor.New(loki.Cfg.Distributor, loki.Cfg.IngesterClient, loki.tenantConfigs, loki.ring, limits, prometheus.DefaultRegisterer)
        if err != nil {
                return loki, nil, nil, err
        } */

        reader := NewReader(loki.Cfg, querier, logger)
        //writer := NewWriter(loki.Cfg, querier, ingesterr, logger)
        writer := NewWriter(loki.Cfg, querier, logger)
        ...
        return loki, store, store.Close, nil
	//return loki, nil
}
...
func (t *Loki) Run(opts RunOpts) error {
	...
	healthy := func() { level.Info(util_log.Logger).Log("msg", "Jaeger-S3 started") }
	stopped := func() { level.Info(util_log.Logger).Log("msg", "Jaeger-S3 stopped") }
	...
	// in other state than New, which should not be the case.
        // 
        // Added by Zaihan
        // we run this is non-blocking to ensure grpc-plugin will run
        c := make(chan bool)
        go func() {
...
// Close writer
func (s *Storag) Close() error {
        err2 := s.writer.Close()
        return err2
}

func (s *Storag) SpanReader() spanstore.Reader {
        return s.reader
}

func (s *Storag) SpanWriter() spanstore.Writer {
        return s.writer
}

func (s *Storag) DependencyReader() dependencystore.Reader {
        return s.reader
}
